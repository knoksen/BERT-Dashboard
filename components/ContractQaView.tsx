...
const ContractQaView: React.FC<ContractQaViewProps> = ({ contractAnalysis, contractText }) => {
  const chatSession = useMemo(() => createContractBertChatSession(contractText), [contractText]);
  ...
  useEffect(() => {
    const sendInitialMessage = async () => {
      if (chatSession && !isInitialMessageSent.current && messages.length === 0) {
        ...
        try {
          const stream = await chatSession.sendMessageStream({ message: "I'm ready to discuss this contract." });
          let fullResponse = '';
