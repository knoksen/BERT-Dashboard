...
import { createDreamBertChatSession } from '../services/geminiService';
...
const DreamInterpretationView: React.FC<DreamInterpretationViewProps> = ({ dreamInterpretation }) => {
  const chatSession = useMemo(() => createDreamBertChatSession(dreamInterpretation), [dreamInterpretation]);
  ...
  useEffect(() => {
    const sendInitialMessage = async () => {
      if (chatSession && !isInitialMessageSent.current && messages.length === 0) {
        ...
        try {
          const stream = await chatSession.sendMessageStream({ message: `I'm ready to discuss my dream, "${dreamInterpretation.title}".` });
          let fullResponse = '';
    ...
    sendInitialMessage();
  }, [chatSession, messages.length, dreamInterpretation.title, setMessages, setIsThinking]);
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 h-[calc(100vh-120px)] flex flex-col">
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border flex flex-col flex-grow">
        <div className="p-4 border-b border-dark-border flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Interpret with DreamBERT</h2>
            <p className="text-dark-text-secondary truncate pr-4">Dream: {dreamInterpretation.title}</p>
          </div>
        </div>
            ...
            {messages.map((msg, index) => {
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 animate-message-in ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.sender === 'bot' && <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center font-bold text-gray-900 text-sm flex-shrink-0"><MoonIcon className="w-5 h-5"/></div>}
        ...
        <div className="p-4 border-t border-dark-border">
          <form onSubmit={handleSend} className="flex items-center gap-2">
            <input
              ...
              placeholder="Ask about symbols, feelings, connections..."
